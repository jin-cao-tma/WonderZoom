...
renderCUDA(
	...
	uint32_t* __restrict__ out_dominant_id)
{
	...
	// 🆕 Track dominant gaussian
	float max_weight = 0.0f;
	uint32_t dominant_id = UINT32_MAX;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// 🆕 Track dominant gaussian by maximum weight
			if (weight > max_weight) {
				max_weight = weight;
				dominant_id = collected_id[j];
			}
	...
	if (inside)
	{
		...
		// 🆕 Output dominant gaussian ID
		out_dominant_id[pix_id] = dominant_id;
	}
}
