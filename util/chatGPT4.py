...
class TextpromptGen(object):
    ...
    def wonder_next_scene(self, style=None, entities=None, scene_name=None, background=None, change_scene_name_by_user=False):
        ...
        if change_scene_name_by_user:
            ...
            print("change scene name", scene_name)
    ...
    def wonder_next_video_scene(self):
        def encode_image_to_base64(image_path):
            with open(image_path, "rb") as f:
                return base64.b64encode(f.read()).decode("utf-8")

        frames_path = "frames/saved_frames/frames"

        # Read images and convert to base64 encoding
        contents = []
        for idx, name in tqdm(enumerate(sorted(os.listdir(frames_path))[::7])):
        ...
        # Base 3D consistency template (concise version)
        BASE_TEMPLATE = "A high-resolution video captured by a real camera moving through a static 3D scene. The camera records a photorealistic environment where all objects maintain rigid geometric structure and fixed spatial relationships. Every surface, wall, and object preserves exact dimensions and proportions as the camera moves. Proper perspective projection and depth relationships throughout the camera motion. No morphing, warping, or dimensional changes in any elements. Consistent material properties and stable textures that respond naturally to viewpoint changes. Architecturally sound geometry with realistic physics and lighting."

        # Call GPT-4o to generate the complete prompt
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {
                    "role": "system",
                    "content": f"""You are a video diffusion prompt generator. Analyze the video frames and generate ONE complete prompt describing the entire video scene.

        Analyze:
        1. **Scene Topic**: What is the main theme/setting of this scene?
        2. **Existing Content**: What's currently visible in the frames?
        3. **Camera Movement**: What direction is the camera moving?
        4. **New Scene Areas**: You can see gray/undefined areas in the frames - what should logically appear in those areas as the camera reveals more of the scene?

        Your prompt must:
        1. Start with this exact template: "{BASE_TEMPLATE}"
        2. Describe the complete scene topic and environment
        3. Describe the camera movement
        4. Naturally describe what appears in the newly revealed areas (without mentioning they were gray/undefined)
        5. Keep everything as ONE flowing paragraph

        Output ONLY the complete prompt text, nothing else."""
                },
                {
                    "role": "user",
                    "content": [{"type": "text", "text": "Generate a complete video diffusion prompt for these frames:"}] + contents
                }
            ],
            max_tokens=800,
        )
        ...
        with open("output/prompts/park_3d.txt", "w") as f:
            f.write(res)
        # Output the complete prompt
        print("="*80)
        print("GENERATED VIDEO DIFFUSION PROMPT:")
        print("="*80)
        print(res)
        print("="*80)